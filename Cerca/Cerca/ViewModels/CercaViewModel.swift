...
internal class CercaViewModel: NSObject, ObservableObject
{
    ...
    */
    ...
    private func shareTokenWithAllPeers() -> Void
    {
        ...
        {
            print("Unable to send token to devices. \(error.localizedDescription)")
        }
...
extension CercaViewModel: MCSessionDelegate
{
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState)
    {
        DispatchQueue.main.async
        {
            switch state
            {
                ...
                @unknown default:
                    fatalError("A new status of the enumeration has appeared. No idea what to do.")
            }
        }
    }
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID)
    {
        ...
        {
            fatalError("The token could not be read from the other device")
        }
